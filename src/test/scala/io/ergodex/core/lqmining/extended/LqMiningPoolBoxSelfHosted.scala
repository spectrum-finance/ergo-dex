...
final class LqMiningPoolBoxSelfHosted[F[_]: RuntimeState](
  ...
  override val constants: Map[Int, Any] = Map(31 -> blake2b256("staking_bundle".getBytes().toVector))
) extends BoxSim[F] {
  override val validatorBytes: String = hex("lm_pool")

  val validator: F[Boolean] =
    withRuntimeState { implicit ctx =>
      // Context (declarations here are only for simulations):
      val BundleScriptHash: Coll[Byte] = getConstant(31).get
      ...
      //   R6[Coll[SigmaProp]]: Redeemers:
      //      0: Main Sigma Proposition;      // Address which can redeem main program budget.
      //      1: Optional Sigma Proposition;  // Address which can redeem optional program budget.
      ...
      val epochNumToEnd  = if (curEpochIx < epochNum) epochNum - curEpochToCalc + 1 else 0
      val epochMainAlloc = prevMainProgramBudget0 / (epochNumToEnd + 1)
      val epochOptAlloc  = prevOptProgramBudget0 / (epochNumToEnd + 1)
      ...
      val validAction = {
        ...
        val prevEpochsCompounded =
          (reservesMainReward.toBigInt + maxRoundingError0 >= epochNumToEnd * epochMainAlloc) &&
          (reservesOptReward.toBigInt + maxRoundingError0 >= epochNumToEnd * epochOptAlloc)
          ...
          if (deltaTMP > 0) // compound
            {
              ...
              val allocMainRem = reservesMainReward - prevMainProgramBudget0 * epochNumToEnd
              val allocOptRem  = reservesOptReward - prevOptProgramBudget0 * epochNumToEnd
              ...
              if (actualTMP > 0) {
                ...
                if (
                  deltaMainReward != 0 || deltaOptReward != 0 || budgetsNotUpdated
                ) // main/optional budget deposit/redeem or epoch budgets update
                  {

                    (prevMainProgramBudget1 == reservesMainReward) &&
                    (prevOptProgramBudget1 == reservesOptReward) &&
                    prevEpochsCompounded
