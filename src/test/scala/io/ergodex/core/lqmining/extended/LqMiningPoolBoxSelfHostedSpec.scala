...
import io.ergodex.core.lqmining.extended.LMPool.ProgramEnded
import io.ergodex.core.lqmining.extended.Token._
import io.ergodex.core.lqmining.extended.generators._
import io.ergodex.core.lqmining.extended.TxBoxes.getDepositTxBoxes
...
//          val mainBudget       = bud.mainBudget
//          val optBudget        = bud.optBudget
...
//          val baseAssetAmount  = lq
  ...
  val redeemLimitDelta = 10
  val mainBudget       = 900L
  val optBudget        = 900L
  val maxRoundingError = 10
  val baseAssetAmount  = 100
  val epochReg         = 8
  val mults            = (1, 2)
...
//    println("pool11 " + pool01)
//    println("pool1 " + pool1)
//    println("pool2 " + pool2)
//    println("pool3 " + pool3)
//    println("pool4 " + pool4)
//
//    println("bundle11.rewardMain " + bundle11.rewardMain)
//    println("bundle11.opt " + bundle11.rewardOpt)
  ...
  //          val (_, Right((pool2, bundle21))) = action1.run(RuntimeCtx.at(startAtHeight)).value
  //          val action3                       = pool2.compound(bundle11, epoch = 1)
  //          val (_, Right((pool3, _, rew)))     = action3.run(RuntimeCtx.at(startAtHeight + epochLen + 1)).value
  ...
  //          if (pool01.conf.epochAlloc - rew.value > 2L) isValid shouldBe false else true
  //        }
  //
