...
final case class CfmmPool[Ledger[_] : RuntimeState](reserves: PoolReserves, config: PoolConfig) {
  ...
  def deposit(inX: Long, inY: AssetInput[Token.Y]): Ledger[VerifiedST[(CfmmPool[Ledger],
    AssetOutput[Token.LP], AssetOutput[Some.type])]] = RuntimeState.withRuntimeState { _ =>
    ...
    Right(
      (
        copy(reserves = reserves.copy(
          reserves.x + inX - changeY,
          reserves.y + inY.value - changeX,
          reserves.lp - unlocked)
        ),
  ...
  def redeem(inLp: AssetInput[Token.LP]): Ledger[VerifiedST[(CfmmPool[Ledger], Long,
    AssetOutput[Token.Y])]] = RuntimeState.withRuntimeState { _ =>
    ...
    Right(
      (
        copy(reserves = reserves.copy(
          reserves.x - redeemedX,
          reserves.y - redeemedY,
          reserves.lp + inLp.value)
        ),
        redeemedX,
  ...
  def swapX(inX: Long): Ledger[VerifiedST[(CfmmPool[Ledger],
    AssetOutput[Token.Y])]] = RuntimeState.withRuntimeState { _ =>
    require(inX > 0)
    val (deltaX, deltaY) =
      (BigInt(inX), BigInt(reserves.y) * inX * config.feeNum /
        (BigInt(reserves.x) * config.feeDenom + inX * config.feeNum))

    Right(
      (
        copy(reserves = reserves.copy(
          reserves.x + deltaX.toLong,
          reserves.y - deltaY.toLong)
        ),
  ...
  def swapY(inY: AssetInput[Token.Y]): Ledger[VerifiedST[(CfmmPool[Ledger], Long)]] =
    RuntimeState.withRuntimeState { _ =>
      require(inY.value > 0)
      val (deltaX, deltaY) =
        (BigInt(reserves.x) * inY.value * config.feeNum /
          (BigInt(reserves.y) * config.feeDenom + inY.value * config.feeNum), BigInt(inY.value))
