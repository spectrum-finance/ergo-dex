...
final case class CfmmPool[Ledger[_]: RuntimeState](reserves: PoolReserves, config: PoolConfig) {
  ...
  def deposit(
    ...
    RuntimeState.withRuntimeState { _ =>
      ...
      Right(
        (
          copy(reserves =
            reserves.copy(reserves.x + inX - changeY, reserves.y + inY.value - changeX, reserves.lp - unlocked)
          ),
  ...
  def redeem(inLp: AssetInput[Token.LP]): Ledger[VerifiedST[(CfmmPool[Ledger], Long, AssetOutput[Token.Y])]] =
    RuntimeState.withRuntimeState { _ =>
  ...
  def swapX(inX: Long): Ledger[VerifiedST[(CfmmPool[Ledger], AssetOutput[Token.Y])]] = RuntimeState.withRuntimeState {
    _ =>
      require(inX > 0)
      val (deltaX, deltaY) =
        (
          BigInt(inX),
          BigInt(reserves.y) * inX * config.feeNum /
  ...
  def swapY(inY: AssetInput[Token.Y]): Ledger[VerifiedST[(CfmmPool[Ledger], Long)]] =
    RuntimeState.withRuntimeState { _ =>
      ...
      Right(
        (copy(reserves = reserves.copy(reserves.x - deltaX.toLong, reserves.y + deltaY.toLong)), deltaX.toLong)
      )
    }
}
