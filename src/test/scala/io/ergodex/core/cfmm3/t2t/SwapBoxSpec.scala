...
class SwapBoxSpec extends AnyFlatSpec with should.Matchers with ScalaCheckPropertyChecks with LedgerPlatform {
  ...
  def getBoxes(swappedAmount: Long, expectedAmount: Long, swappedToken: String, recToken: String): (UserBox[Ledger], SwapBox[Ledger], MinerBox[Ledger]) = {
    ...
    val userBox = new UserBox(
      ...
      tokens = Vector(
        tokenId(recToken) -> expectedAmount,
      ),
      registers = Map(
      )
    )
  ...
  it should "validate swap y behaviour mirrored from simulation" in {
    ...
    val (_, isValidSwapY) = swapBox1.validator.run(RuntimeCtx(startAtHeight, inputs = List(poolBox0, swapBox1),
      outputs = List(poolBox1, userBox1, minerBox1), vars = Map(0 -> 0, 1 -> false))).value
  ...
  it should "validate swap x behaviour mirrored from simulation" in {
    ...
    val (_, isValidSwapY) = swapBox1.validator.run(RuntimeCtx(startAtHeight, inputs = List(poolBox0, swapBox1),
      outputs = List(poolBox1, userBox1, minerBox1), vars = Map(0 -> 1, 1 -> false))).value
    val (_, isValidPool) = poolBox0.validator.run(RuntimeCtx(startAtHeight, outputs = List(poolBox1))).value
  ...
  it should "validate illegal swap behaviour" in {
    ...
    val (userBox1, swapBox1, minerBox1) = getBoxes(inputX.value, expectedYAmount - 1, "x", "y")
  ...
  it should "validate swap behaviour then x is spf mirrored from simulation" in {
    ...
    val (_, isValidSwapY) = swapBox1.validator.run(RuntimeCtx(startAtHeight, inputs = List(poolBox0, swapBox1),
      outputs = List(poolBox1, userBox1, minerBox1), vars = Map(0 -> 1, 1 -> true))).value
    val (_, isValidPool) = poolBox0.validator.run(RuntimeCtx(startAtHeight, outputs = List(poolBox1))).value
  ...
  it should "validate swap behaviour then y is spf mirrored from simulation" in {
    ...
    val (userBox1, swapBox1, minerBox1) = getBoxes(inputY.value, expectedYAmount, "y", "x")


    val (_, isValidSwapY) = swapBox1.validator.run(RuntimeCtx(startAtHeight, inputs = List(poolBox0, swapBox1),
      outputs = List(poolBox1, userBox1, minerBox1), vars = Map(0 -> 0, 1 -> true))).value
    val (_, isValidPool) = poolBox0.validator.run(RuntimeCtx(startAtHeight, outputs = List(poolBox1))).value
  ...
  it should "validate illegal swap behaviour then y is spf mirrored from simulation" in {
    ...
    val (userBox1, swapBox1, minerBox1) = getBoxes(inputY.value, expectedYAmount - 1, "y", "x")
