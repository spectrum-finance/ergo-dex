...
final case class CfmmPool[Ledger[_] : RuntimeState](reserves: PoolReserves, config: PoolConfig) {
  ...
  def deposit(inX: AssetInput[Token.X], inY: AssetInput[Token.Y]): Ledger[VerifiedST[(CfmmPool[Ledger],
  ...
  def redeem(inLp: AssetInput[Token.LP]): Ledger[VerifiedST[(CfmmPool[Ledger], AssetOutput[Token.X],
  ...
  def swapX(inX: AssetInput[Token.X]): Ledger[VerifiedST[(CfmmPool[Ledger],
    AssetOutput[Token.Y])]] = RuntimeState.withRuntimeState { _ =>
    require(inX.value > 0)
    val (deltaX, deltaY) =
      (BigInt(inX.value), BigInt(reserves.y) * inX.value * config.feeNum /
